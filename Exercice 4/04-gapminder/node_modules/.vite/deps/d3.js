...
import {
  ...
  tsv,
  xml_default
} from "./chunk-TD5KAJSK.js";
import "./chunk-4SAMJQUC.js";
import {
  active_default,
  dispatch_default,
  interrupt_default,
...
function ringContains(ring, point6) {
  ...
  for (var i = 0, n = ring.length, j = n - 1; i < n; j = i++) {
    ...
    if (segmentContains(pi4, pj, point6)) return 0;
  ...
  *neighbors(i) {
  ...
  _clipSegment(x0, y0, x1, y1, c0, c1) {
    ...
    while (true) {
      ...
      let x4, y4, c6 = c0 || c1;
      if (c6 & 8) x4 = x0 + (x1 - x0) * (this.ymax - y0) / (y1 - y0), y4 = this.ymax;
      else if (c6 & 4) x4 = x0 + (x1 - x0) * (this.ymin - y0) / (y1 - y0), y4 = this.ymin;
      else if (c6 & 2) y4 = y0 + (y1 - y0) * (this.xmax - x0) / (x1 - x0), x4 = this.xmax;
  ...
  _project(x0, y0, vx, vy) {
    ...
    if (vy < 0) {
      ...
      if ((c6 = (this.ymin - y0) / vy) < t) y4 = this.ymin, x4 = x0 + (t = c6) * vx;
    } else if (vy > 0) {
      if (y0 >= this.ymax) return null;
      if ((c6 = (this.ymax - y0) / vy) < t) y4 = this.ymax, x4 = x0 + (t = c6) * vx;
    }
    if (vx > 0) {
      if (x0 >= this.xmax) return null;
      if ((c6 = (this.xmax - x0) / vx) < t) x4 = this.xmax, y4 = y0 + (t = c6) * vy;
    } else if (vx < 0) {
      if (x0 <= this.xmin) return null;
      if ((c6 = (this.xmin - x0) / vx) < t) x4 = this.xmin, y4 = y0 + (t = c6) * vy;
    }
    return [x4, y4];
  }
  _edgecode(x4, y4) {
  ...
  function arc() {
    ...
    if (!(r1 > epsilon5)) context.moveTo(0, 0);
      ...
      if (rp > epsilon5) {
        ...
        if ((da0 -= p0 * 2) > epsilon5) p0 *= cw ? 1 : -1, a00 += p0, a10 -= p0;
